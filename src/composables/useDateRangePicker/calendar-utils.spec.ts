...
describe("isSameDay", () => {
  ...
  it("returns false for same day different month", () => {
    expect(isSameDay(new Date(2026, 3, 15), new Date(2026, 4, 15))).toBe(
      false,
    );
  });

  it("returns false for same day different year", () => {
    expect(isSameDay(new Date(2026, 3, 15), new Date(2025, 3, 15))).toBe(
      false,
    );
  });
});
...
describe("isSameMonth", () => {
  it("returns true when date is in the given year/month", () => {
    expect(isSameMonth(new Date(2026, 3, 15), { year: 2026, month: 4 })).toBe(
      true,
    );
  });

  it("returns false for different month", () => {
    expect(isSameMonth(new Date(2026, 3, 15), { year: 2026, month: 3 })).toBe(
      false,
    );
  });

  it("returns false for different year same month", () => {
    expect(isSameMonth(new Date(2025, 3, 15), { year: 2026, month: 4 })).toBe(
      false,
    );
  });
});
...
describe("generateMonthGrid", () => {
  ...
  it("contains all days of the month", () => {
    ...
    const aprilDays = grid
      .flat()
      .filter((d) => d.getMonth() === 3 && d.getFullYear() === 2026);
  ...
  it("handles February in a leap year", () => {
    ...
    const febDays = grid
      .flat()
      .filter((d) => d.getMonth() === 1 && d.getFullYear() === 2024);
  ...
  it("handles February in a non-leap year", () => {
    ...
    const febDays = grid
      .flat()
      .filter((d) => d.getMonth() === 1 && d.getFullYear() === 2026);
